...
extern uint16_t img[120][160];

uint8_t HAL_SPI_Transmit(SPI_TypeDef* hspi, uint8_t* pData, uint32_t len, uint32_t timeout)
{
    //直接发送
    // for (int i = 0; i < len; i++) {
    //     SPI_SendData(SPI2, pData[i]);
    //     while (SPI_GetFlagStatus((SPI_TypeDef*)hspi, SPI_FLAG_TXEPT) == RESET) {
    //         if (timeout-- == 0) {
    //             return 1;
    //         }
    //     }
    // }
    MM_SPI_TransmitReceive(SPI2, pData, NULL, len, HAL_MAX_DELAY);
    return 0;
}

void HAL_GPIO_WritePin(GPIO_TypeDef* gpio, uint16_t pin, uint8_t state)
{
    switch (state) {
    case GPIO_PIN_RESET:
...
#define TFT_CS_H() GPIO_SetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin)
#define TFT_CS_L() GPIO_ResetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin)
...
static uint8_t _data_rotation[4]
    = { ST7735_MADCTL_MX, ST7735_MADCTL_MY, ST7735_MADCTL_MV, ST7735_MADCTL_RGB };
#endif
...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data)
{
    ...
    for (int i = 0; i < w * h; i++) {
        ST7735_WriteData((uint8_t*)(data + i), sizeof(uint16_t));
        // SPI_SendData(SPI2, *(uint8_t*)(data + i));
        // while (SPI_GetFlagStatus((SPI_TypeDef*)SPI2, SPI_FLAG_TXEPT) == RESET) {
        //     if (timeout-- == 0) {
        //         break;
        //     }
        // }
