...
uint8_t AC_static = 0, beep_static = 0, AP_static = 0, Humidifier_static = 0, Hood_static = 0;
...
//				TIM3_ENABLE_30S(); // PING定时器重回30s的时间
	...
	{												  // if成立的话，说明命令缓冲区有数据了
		...
		//自己写的
		//其中，重点是strstr()函数，不懂网上找查找
		//重点，判断内容时，"前要加\来识别，否则会识别错

		//白灯
		if (strstr((char *)MQTT_CMDOutPtr + 2, "\"data\":1,\"cmd\":\"led\""))
		{
//			LED_WHITE;
		}
		else if (strstr((char *)MQTT_CMDOutPtr + 2, "\"data\":0,\"cmd\":\"led\""))
		{
//			LED_RGBOFF;
		}
