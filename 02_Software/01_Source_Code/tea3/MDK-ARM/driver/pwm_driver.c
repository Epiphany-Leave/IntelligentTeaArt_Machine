...
//	pwm_bsp_init();
	...
	// 注册舵机实例（必须在pwm_driver_init之后）
    Servo_Register(&htim2, TIM_CHANNEL_1);  // 舵机1
    Servo_Register(&htim2, TIM_CHANNEL_2);  // 舵机2
	Servo_Register(&htim2, TIM_CHANNEL_3);  // 舵机3
	Servo_Register(&htim2, TIM_CHANNEL_4);  // 舵机4
	Servo_Register(&htim3, TIM_CHANNEL_1);  // 舵机5
//	Servo_StartSmoothMove(&htim2, TIM_CHANNEL_1, (HOLDER_1), 1000);
//	Servo_StartSmoothMove(&htim2, TIM_CHANNEL_2, (HOLDER_2), 1000);
//	Servo_StartSmoothMove(&htim2, TIM_CHANNEL_3, (HOLDER_3), 1000);
//	Servo_StartSmoothMove(&htim3, TIM_CHANNEL_2, (HOLDER_4), 1000);
//	Servo_StartSmoothMove(&htim2, TIM_CHANNEL_1, (HOLDER_1_M), 500);
	Servo_SetInstantAngle(&htim2, TIM_CHANNEL_1, (HOLDER_1_M));
	Servo_SetInstantAngle(&htim2, TIM_CHANNEL_2, (HOLDER_2_U));
	Servo_SetInstantAngle(&htim2, TIM_CHANNEL_3, (HOLDER_3_U));
	Servo_SetInstantAngle(&htim2, TIM_CHANNEL_4, (HOLDER_4_P));
	Servo_SetInstantAngle(&htim3, TIM_CHANNEL_1, (HOLDER_5_P));
}
...
uint8_t Servo_Register(TIM_HandleTypeDef *htim, uint32_t channel) 
{
	...
	{
        if(!servoControls[i].is_initialized) 
		{
            servoControls[i].htim = htim;
            servoControls[i].channel = channel;
            servoControls[i].is_initialized = 1;
            
            // 读取当前PWM值初始化角度
            uint32_t ccr = __HAL_TIM_GET_COMPARE(htim, channel);
            servoControls[i].current_angle = (ccr - 500) * 0.09f; // 0.09=180/2000
            return 1;
        }
    }
    return 0;
                              ...
                              float target_angle, uint16_t duration_ms)
{
	...
	{
		uint32_t ccr = __HAL_TIM_GET_COMPARE(htim, channel);
		servoControls[slot].current_angle = (ccr - 500) * 0.09f; // 根据实际PWM值反推角度
	}
